...
def handle_siri_post(uuid, data: dict):
    ...
    siri = data["Siri"]
    if "HeartbeatNotification" in siri:
        timestamp = datetime.fromisoformat(
            siri["HeartbeatNotification"]["RequestTimestamp"]
        )
    else:
    ...
    stats = cache.get(key, [])
    stats.append({"datetime": timezone.now(), "timestamp": timestamp})
    stats = stats[-50:]
...
def refresh_dvla_tax_statuses():
    api_key = getattr(settings, "DVLA_VEHICLE_ENQUIRY_API_KEY", "")
    if not api_key:
        return
    ...
    vehicles = Vehicle.objects.filter(vor=False).exclude(reg="")
    ...
    for vehicle in vehicles.iterator():
        try:
            ...
            logger.warning(
                "DVLA lookup failed for vehicle %s (%s): %s",
                vehicle.pk,
                vehicle.reg,
                exc,
            )
        ...
        update = build_dvla_update(vehicle, payload, checked_at=checked_at)
        vehicle.dvla_tax_status = update["tax_status"]
        vehicle.dvla_mot_status = update["mot_status"]
        vehicle.dvla_euro_status = update["euro_status"]
        vehicle.dvla_tax_status_checked_at = update["checked_at"]
        vehicle.save(
            update_fields=[
                "dvla_tax_status",
                "dvla_mot_status",
