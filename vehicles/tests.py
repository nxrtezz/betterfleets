...
class VehiclesTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        organisation = Organisation.objects.create(
            name="Madrigal Holdings",
            slug="madrigal-holdings",
        )
        group = OperatorGroup.objects.create(
            name="Madrigal Electromotive",
            slug="madrigal-electromotive",
            organisation=organisation,
            description="Main operating division for the eastern business.",
            website="https://example.com/division",
            social_x="https://x.example/madrigal",
            social_fb="https://facebook.example/madrigal",
            social_instagram="https://instagram.example/madrigal",
            header_background="#123456",
            header_foreground="#f8fafc",
            accent_colour="#22c55e",
            custom_css=".division-profile__banner{min-height:8rem;}",
        )
        ...
        Depot.objects.create(
            operator=cls.lynx,
            name="King's Lynn Depot",
            location=Point(0.4, 52.8),
        )
        ...
        cls.withdrawn_livery = Livery.objects.create(
            name="heritage cream",
            colours="#f4e6b2 #7d1f1f",
            colour="#f4e6b2",
            published=True,
        )
    ...
    def test_parent(self):
        ...
        self.assertContains(response, "Main operating division for the eastern business.")
        self.assertContains(response, "https://example.com/division")
        self.assertContains(response, "https://x.example/madrigal")
    ...
    def test_preservation_group_pages(self):
        ...
        preserved_vehicle = Vehicle.objects.create(
            code="PV01",
            reg="PV01ABC",
            preserved=True,
            preservation_group=group,
        )

        response = self.client.get("/preservation-groups/")
        self.assertEqual(response.status_code, HTTPStatus.OK)
        self.assertContains(response, "Vintage Fleet")
        ...
        self.assertEqual(response.status_code, HTTPStatus.OK)
        self.assertNotContains(response, "PV01ABC")

        response = self.client.get(f"{group.get_absolute_url()}?tab=vehicles")
        self.assertEqual(response.status_code, HTTPStatus.OK)
        self.assertContains(response, "Preserved Vehicles")
        self.assertContains(response, "PV01ABC")
        self.assertContains(response, preserved_vehicle.get_absolute_url())
    ...
    def test_fleet_lists(self):
        ...
        self.assertContains(response, "/operators/lynx/map")
        self.assertContains(response, "Fleet liveries")
        self.assertContains(response, "black with lemon piping")
        self.assertNotContains(response, "heritage cream")
        self.assertContains(response, "/organisations/madrigal-holdings")
        self.assertContains(response, "?tab=depots")
        self.assertNotContains(response, "Depot map</li>")

        response = self.client.get("/operators/lynx/vehicles?tab=depots")
        self.assertContains(response, "Depot map")
        self.assertContains(response, "King&#x27;s Lynn Depot")
        ...
        response = self.client.get("/operators/lynx/vehicles?withdrawn=1")
        self.assertContains(response, "heritage cream")
...
class VehicleCodeFleetDisplayTests(TestCase):
    def test_code_matches_fleet_number_and_fleet_code(self):
        v = Vehicle(code="50", fleet_number=50)
        v.save()
        self.assertTrue(v.code_matches_fleet_primary())

    def test_code_mismatches_when_composite(self):
        v = Vehicle(code="502_-_DK09_DZH", fleet_number=502, fleet_code="502")
        self.assertFalse(v.code_matches_fleet_primary())

    def test_code_matches_underscores_to_fleet_code_spaces(self):
        v = Vehicle(code="G_2434", fleet_code="G 2434")
        self.assertTrue(v.code_matches_fleet_primary())

    def test_get_code_display_replaces_underscores(self):
        self.assertEqual(
            Vehicle(code="502_-_DK09_DZH").get_code_display(),
            "502 - DK09 DZH",
        )
