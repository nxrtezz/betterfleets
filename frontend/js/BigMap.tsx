...
function TripSidebar(props: {
  ...
  const operator = trip.operator ? (
    <li>
      <a href={`/operators/${trip.operator.slug}/vehicles`}>{trip.operator.name}</a>
    </li>
  ) : null;
...
export default function BigMap(
  ...
  const loadVehicles = React.useCallback(
    async (first = false) => {
      ...
      if (allVehicles.length || vehiclesLength.current || first) {
        ...
        vehiclesLength.current = allVehicles.length;
