...
export default function VehiclePopup({
  ...
  // Check if this is a train (has train-specific fields)
  const isTrain = !!(item as any).headcode || !!(item as any).uid || !!(item as any).toc_code;

  let line_name: ReactNode;
  let vehicle: ReactNode;
  let operator: ReactNode;

  if (isTrain) {
    // Train-specific display
    const trainItem = item as any;
    line_name = trainItem.headcode || "";
    if (trainItem.destination) {
      if (line_name) {
        line_name += " to ";
      }
      line_name += trainItem.destination;
    }
    if (trainItem.origin) {
      line_name = `${trainItem.origin} - ${line_name}`;
    }

    vehicle = trainItem.vehicle?.name || trainItem.operator || "Train";
    
    operator = trainItem.train_operator || trainItem.operator?.name;
  } else {
    // Bus-specific display
    ...
    if (item.vehicle?.fleet_number || item.vehicle?.reg) {
      vehicle = [item.vehicle.fleet_number, item.vehicle.reg]
        .filter(Boolean)
        .join(" - ");
    }
    if (item.vehicle?.url) {
    ...
    if (item.operator?.url) {
      operator = <a href={item.operator.url}>{operator}</a>;
    }
  }
  ...
  return (
    ...
    >
      ...
      {!isTrain && item.vehicle?.vehicle_type && <div>{item.vehicle.vehicle_type}</div>}
      {!isTrain && item.vehicle?.features && (
