...
def resolve_trip_query(identifier, queryset=None):
    trips = queryset if queryset is not None else Trip.objects.all()
    ...
    if identifier.isdigit():
        trip = trips.filter(pk=int(identifier))
        if trip.exists():
            return trip

    trip = trips.filter(
        Q(ticket_machine_code=identifier) | Q(vehicle_journey_code=identifier)
    )
    if trip.count() <= 1:
        return trip

    todays_trips = trip.filter(calendar__in=get_calendars(localdate()))
    if todays_trips.count() == 1:
        return todays_trips

    return trip


def resolve_trip(identifier, queryset=None):
    trip = resolve_trip_query(identifier, queryset=queryset).order_by("-id").first()
    if trip is None:
