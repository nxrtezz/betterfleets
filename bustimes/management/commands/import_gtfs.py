...
class Command(BaseCommand):
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--file",
            type=str,
            help="Path to a local GTFS .zip file to import",
        )
        parser.add_argument(
            "--operator",
            type=str,
            help="NOC code of the operator to assign the data to",
        )
    ...
    def handle_operator(self, line):
        ...
        # If --operator flag was provided, use that operator
        if hasattr(self, 'specified_operator') and self.specified_operator:
            return self.specified_operator
    ...
    def do_stops(self, feed: gtfs_kit.feed.Feed) -> dict[str, StopPoint]:
        ...
        for stop in stops_to_create:
            ...
            if admin_area_id not in admin_areas:
                # Only check if admin_area_id is numeric (ATCO format)
                if admin_area_id.isdigit():
                    admin_areas[admin_area_id] = AdminArea.objects.filter(
                        id=admin_area_id
                    ).exists()
                else:
                    admin_areas[admin_area_id] = False
        ...
        # Query all stops from the source and build dictionary keyed by atco_code
        all_stops = StopPoint.objects.filter(
            atco_code__in=stops.keys()
        ).only("atco_code", "latlong")
        return {stop.atco_code: stop for stop in all_stops}
    ...
    def handle_zipfile(self, path):
        ...
        for line in feed.trips.itertuples():
            ...
            calendar = calendars.get(line.service_id)
            trips[line.trip_id] = Trip(
                route=route,
                calendar=calendar,
                inbound=direction_id == 1 if direction_id is not None else False,
        ...
        with (
            ...
            cursor.copy(
                "COPY bustimes_stoptime (stop_code, arrival, departure, sequence, trip_id, timing_status, pick_up, set_down) FROM STDIN"
            ) as copy,
        ):
            for line in feed.stop_times.itertuples():
                timepoint = getattr(line, "timepoint", 1)
                timing_status = "PTP" if not pd.isna(timepoint) and timepoint == 1 else "OTH"

                pick_up = None
                pickup_type = getattr(line, "pickup_type", 0)
                if not pd.isna(pickup_type):
                    match pickup_type:
                ...
                stop_point = stops.get(line.stop_id)
                stop_code = stop_point.atco_code if stop_point else line.stop_id
        ...
        for service in services:
            # Skip do_stop_usages() for GTFS imports as StopTime records don't have proper stop foreign keys
    ...
    def handle(self, *args, **options):
        # Handle --file flag for local GTFS file import
        if options["file"]:
            ...
            if options["operator"]:
                operator = Operator.objects.filter(noc=options["operator"]).first()
                if not operator:
                    self.stderr.write(f"Operator not found with NOC: {options['operator']}")
                    return
                self.specified_operator = operator

            # Create or get a DataSource for this file
            source_name = file_path.stem
            source, created = DataSource.objects.get_or_create(
                name=source_name,
                defaults={"url": f"file://{file_path}"}
            )
            self.source = source

            logger.info(f"Importing GTFS from {file_path}")
            try:
                with log_time_taken(logger):
                    self.handle_zipfile(file_path)
            except (OSError, BadZipFile) as e:
                logger.exception(e)
                self.stderr.write(f"Error processing file: {e}")
            return

        # Original behavior for collections
