...
class Command(BaseCommand):
    ...
    def do_file(self, name, details):
        # Use filter().first() to handle duplicate DataSource records
        source = DataSource.objects.filter(url=f"ftp://{self.ftp.host}/{name}").first()
        if not source:
            source = DataSource.objects.create(url=f"ftp://{self.ftp.host}/{name}")
