...
def get_operator_name(operator_element):
    ...
    for element_name in ("TradingName", "OperatorShortName"):
...
def get_registration(service_code: str) -> None:
    # Registration model doesn't exist - return None
    # This function was used for looking up bus service registrations
    return None
...
class Command(BaseCommand):
    ...
    def add_arguments(parser):
        ...
        parser.add_argument(
            "--end-date",
            type=str,
            help="Override timetable expiry date (YYYY-MM-DD format) for preservation imports",
        )
    ...
    def handle(self, *args, **options):
        ...
        if options.get("end_date"):
            try:
                self.end_date_override = datetime.datetime.strptime(
                    options["end_date"], "%Y-%m-%d"
                ).date()
            except ValueError:
                self.stderr.write(
                    self.style.ERROR(
                        f"Invalid end_date format: {options['end_date']}. Use YYYY-MM-DD format."
                    )
                )
                return
    ...
    def set_region(self, archive_name):
        ...
        if self.region_id:
            ...
            if not Region.objects.filter(pk=self.region_id).exists():
                logger.warning(f"Region {self.region_id} does not exist in database, skipping region assignment")
                self.region_id = None
    ...
    def get_operator(self, operator_element):
        ...
        if operator_code:
            if self.source.name == "L":
                operator = get_operator_by("L", operator_code)
                if operator:
                    return operator
            else:
            ...
            with zipfile.ZipFile(archive_path) as archive:
                ...
                if self.show_progress:
                    with Progress(
                        ...
                        TimeElapsedColumn(),
                        TextColumn("Region: {task.fields[region]}"),
                        TextColumn("Operator: {task.fields[operator]}"),
                        TextColumn("Timer: {task.fields[timer]}"),
                        expand=True,
                    ) as progress:
                        task = progress.add_task(
                            basename,
                            total=len(items),
                            region=self.region_id or basename,
                            operator="",
                            timer="0m 0s",
                        )
                        for filename in items:
                            ...
                            timer_str = f"{minutes}m {seconds}s"
                            operator_str = self.current_operator or "Unknown"
                            progress.update(
                                task,
                                advance=1,
    ...
    def should_preserve_existing_service_operators(self, service, existing) -> bool:
        return bool(self.source.is_tnds() and existing and service.operator.exists())
    ...
    def get_calendar(self, operating_profile, operating_period):
        ...
        calendar = Calendar(
            ...
            end_date=self.end_date_override or operating_period.end,
            source=self.source,
        )
    ...
    def should_defer_to_other_source(self, operators: dict, line_name: str):
        # Always return False to import all routes regardless of other sources
        return False
    ...
    def handle_service(
        ...
        # Update current operator for progress display
        if operators:
            first_operator = next(iter(operators.values()))
            if first_operator:
                self.current_operator = f"{first_operator.noc} | {first_operator.name}"
        ...
        if self.source.name.startswith("Arriva") and "tfl_" in filename:
            logger.info(
        ...
        for line in txc_service.lines:
            ...
            if operators and not self.should_preserve_existing_service_operators(
                service, existing
            ):
            ...
            route_defaults = {
                ...
                "end_date": self.end_date_override or txc_service.operating_period.end,
                "service": service,
