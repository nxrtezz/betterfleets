...
class ImportTransXChangeTest(TestCase):
    ...
    def test_do_service_scotland(self):
        ...
        # Even if the service is flagged as problematic, render the timetable
        # when usable route/trip data still exists.
        service.timetable_wrong = True
        service.save(update_fields=["timetable_wrong"])
        res = self.client.get(service.get_absolute_url())
        self.assertContains(res, 'class="timetable-date"')
    ...
    def test_bods_service_defers_to_existing_tnds_service(self):
        ...
        command.source = DataSource.objects.create(
            name="First East",
            url="https://data.bus-data.dft.gov.uk/timetable/download/1/",
        )

        self.assertTrue(
            command.should_defer_to_other_source({"FECS": self.fecs}, "12")
        )

    def test_tnds_service_does_not_defer_to_existing_bods_service(self):
    ...
    def test_tnds_matched_service_preserves_existing_operators(self):
        command = import_transxchange.Command()
        command.set_up()
        command.source = DataSource.objects.create(
            name="EA",
        ...
        self.assertTrue(command.should_preserve_existing_service_operators(service, True))
        self.assertFalse(command.should_preserve_existing_service_operators(service, False))

    def test_finish_services_does_not_normalize_operators_for_tnds(self):
        command = import_transxchange.Command()
        command.set_up()
        command.service_ids = set()
        command.source = DataSource.objects.create(
        ...
        service.operator.add(self.fecs, self.fabd)
        command.service_ids.add(service.id)

        command.finish_services()

        self.assertEqual(
            list(service.operator.order_by("noc").values_list("noc", flat=True)),
            ["FABD", "FECS"],
        )
