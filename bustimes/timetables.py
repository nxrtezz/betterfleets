...
from .models import Calendar, Note, StopTime, Trip, day_keys
...
class Timetable:
    def __init__(self, routes, date=None, calendar_id=None, detailed=False, operators=None, day_of_week=None):
        ...
        if self.calendars and not self.calendar:
            if self.day_of_week:
                # Filter calendars by day of week
                self.calendar_ids = [
                    calendar.id for calendar in self.calendars 
                    if getattr(calendar, self.day_of_week.lower())
                ]
            else:
        ...
        if self.date:
            ...
            # For day of week, use today for route revision filtering
            self.current_routes = get_routes(routes, self.today)

        if not self.calendar:
            if self.calendars:
                if self.day_of_week:
                    # Already filtered calendar_ids above
                    pass
                else:
        ...
        if not self.calendar:
            if self.calendars:
                if self.day_of_week:
                    ...
                    day_field = self.day_of_week.lower()
                    trips = trips.filter(
                        Q(**{f"calendar__{day_field}": True}) | Q(calendar=None)
                    )
                else:
                    trips = trips.filter(
    ...
    def get_day_options(self):
        ...
        day_order = {day: i for i, day in enumerate(day_keys)}
