    ...
    *,
    ...
    for trip in trips.itertuples():
        ...
        for a, b in pairwise(trip_stop_times.itertuples()):
            ...
            # Rail station centroids can be a little further from the track than roadside
            # stops are from a road shape, so allow a slightly looser tolerance there.
            max_distance = 0.02 if mode == "rail" else 0.01

            # skip if either stop is too far from the route geometry
            projected_a = trip.geometry.interpolate(start_dist)
            projected_b = trip.geometry.interpolate(end_dist)
            if (
                point_a.distance(projected_a) > max_distance
                or point_b.distance(projected_b) > max_distance
            ):
                start_dist = None
                continue
