...
class Route(models.Model):
    ...
    def get_date_display(self) -> str:
        """Return date display, showing day names if timetable is out of date."""
        if not self.end_date:
            return f"{self.start_date}" if self.start_date else ""
        
        today = localdate()
        if self.end_date >= today:
            # Not out of date, return normal date range
            return f"{self.start_date}–{self.end_date}"
        
        # Out of date - try to get days from associated calendars
        from .models import Calendar
        calendars = Calendar.objects.filter(trip__route=self).distinct()
        if calendars.exists():
            # Get days from the first calendar
            calendar = calendars.first()
            days = calendar.get_days()
            if days:
                return " ".join(days)
        
        # Fallback to showing dates
        return f"{self.start_date}–{self.end_date}"
...
class Calendar(models.Model):
    ...
    def describe_for_timetable(self, today=None) -> str:
        ...
        # If timetable is out of date, skip date-specific additions
        # but only if it's not a single-day timetable (already handled above)
        if end_date and end_date < today and start_date != end_date:
            return description
    ...
    def get_date_display(self) -> str:
        ...
        today = localdate()
        if self.end_date >= today:
            # Not out of date, return normal string representation
            return str(self)
        
        # Out of date - return just the day names
        days = self.get_days()
        if not days:
            return self.summary
        return " ".join(days)
...
class Garage(models.Model):
    operators = models.ManyToManyField(
        "busstops.Operator", blank=True
    )
    external_id = models.CharField(max_length=100, blank=True, null=True, unique=True)
    code = models.CharField(max_length=50, blank=True)
    name = models.CharField(max_length=100, blank=True)
    region = models.ForeignKey("busstops.Region", models.SET_NULL, null=True, blank=True)
...
class RouteWaypoint(models.Model):
    """
    Waypoints for routing between stops.
    
    Waypoints are routing hints that guide OSRM to choose specific roads
    when generating route geometry between two stops. Each waypoint belongs
    to a RouteLink (segment between two stops) and has an order field to
    maintain the sequence of waypoints within that segment.
    
    When generating OSRM routes, coordinates are sent in order:
    Stop A -> Waypoint 1 -> Waypoint 2 -> ... -> Stop B
    
    This ensures waypoints only affect their assigned segment and don't
    interfere with other parts of the route.
    """
    ...
    latitude = models.FloatField(
        help_text='Waypoint latitude coordinate'
    )
    longitude = models.FloatField(
        help_text='Waypoint longitude coordinate'
    )
    order = models.PositiveSmallIntegerField(
        help_text='Order of this waypoint within the segment (0, 1, 2, ...)'
    )
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['route_link', 'order']),
        ]

    def __str__(self):
        return f"Waypoint {self.order} for {self.route_link}"

    @property
    def coordinates(self):
        """Return coordinates as (longitude, latitude) tuple for OSRM."""


class SimulationConfig(models.Model):
    """
    Configuration for simulating vehicle tracking on services without live GPS data.
    """
    service = models.ForeignKey(
        "busstops.Service",
        models.CASCADE,
        related_name='simulation_configs',
        help_text='The service to simulate tracking for'
    )
    enabled = models.BooleanField(
        default=False,
        help_text='Whether simulation is enabled for this service'
    )
    vehicle_count = models.PositiveSmallIntegerField(
        default=1,
        help_text='Number of vehicles to simulate for this service'
    )
    destination_inbound = models.CharField(
        max_length=255,
        blank=True,
        help_text='Custom destination name for inbound trips'
    )
    destination_outbound = models.CharField(
        max_length=255,
        blank=True,
        help_text='Custom destination name for outbound trips'
    )
    vehicle_config = models.JSONField(
        default=dict,
        blank=True,
        help_text='Vehicle configuration data (IDs, registrations, names)'
    )
    created_at = models.DateTimeField(auto_now_add=True)
    modified_at = models.DateTimeField(auto_now=True)
    ...
    def __str__(self):
        return f"Simulation config for {self.service}"

    def get_vehicle_configs(self):
        """
        Return list of vehicle configurations.
        Generates default configs if not set.
        """
        if self.vehicle_config:
            return self.vehicle_config
        
        # Generate default vehicle configs
        configs = []
        for i in range(self.vehicle_count):
            configs.append({
                'id': -(i + 1),
                'reg': f"SIM{self.service.id:04d}-{i+1:02d}",
