...
class StopPointTests(TestCase):
    ...
    def test_stop_features_are_grouped_by_category(self):
        shelter = StopFeature.objects.create(name="Shelter")
        wheelchair = StopFeature.objects.create(
            name="Step-free access",
            category=StopFeature.Category.ACCESSIBILITY,
        )
        self.cardiff_airport_stop.features.add(shelter, wheelchair)

        self.assertEqual(
            list(self.cardiff_airport_stop.get_standard_features()),
            [shelter],
        )
        self.assertEqual(
            list(self.cardiff_airport_stop.get_accessibility_features()),
            [wheelchair],
        )
