...
TICKET_HINT_TOKEN_RE = re.compile(r"[A-Za-z']+")
IGNORED_TICKET_HINT_WORDS = {
    "adult",
    "app",
    "bus",
    "child",
...
def resolve_fare_table_tariff(fare_table_element, tariffs, filename):
    tariff_ref = fare_table_element.find("usedIn/TariffRef")
    if tariff_ref is not None:
        tariff_code = tariff_ref.attrib["ref"]
        return tariff_code, tariffs[tariff_code]

    if len(tariffs) == 1:
        tariff_code, tariff = next(iter(tariffs.items()))
        return tariff_code, tariff

    logger.warning(
        "Skipping fare table without tariff ref in %s: %s",
        filename,
        fare_table_element.attrib.get("id", ""),
    )
    return None, None
...
@cache
def get_services(operator_noc, line_name):
    return tuple(
        Service.objects.filter(
            Q(Exists("route", filter=Q(line_name__iexact=line_name)))
            | Q(line_name__iexact=line_name),
            operator__noc=operator_noc,
            current=True,
        ).distinct()
    )


def get_hint_tokens(*values):
    tokens = []
    for value in values:
        for token in TICKET_HINT_TOKEN_RE.findall(value or ""):
            token = token.lower()
            if len(token) < 4 or token in IGNORED_TICKET_HINT_WORDS:
                continue
...
def get_matching_services(operator, line_names, *hint_values):
    matched_services = []
    hint_tokens = get_hint_tokens(*hint_values)

    for line_name in line_names:
        services = list(get_services(operator.noc, line_name))
        if len(services) > 1 and hint_tokens:
            hinted_services = [
                service
                for service in services
                if any(
                    token in f"{service.description} {service.line_brand}".lower()
                    for token in hint_tokens
                )
            ]
            if hinted_services:
                services = hinted_services
...
class Command(BaseCommand):
    ...
    def handle_file(self, source, open_file, filename=None):
        ...
        for tariff_element in element.findall(
            ...
            tariff_elements[tariff.code] = tariff_element
            ...
            if operator_ref is not None:
                try:
                    ...
                    if line_ref is not None:
                        line = lines.get(line_ref.attrib["ref"])
                        if line is not None and line.findtext("PublicCode"):
                            line_names.append(line.findtext("PublicCode"))
                    elif lines:
                        line_names = [
                            line.findtext("PublicCode")
                            for line in lines.values()
                            if line.findtext("PublicCode")
                        ]

                    matched_services = get_matching_services(
                        operator,
                        line_names,
                        tariff.name,
                        source.name,
                    ...
                    if matched_services:
                        tariff.services.add(*matched_services)
        ...
        for fare_table_element in element.findall(
            ...
            tariff_ref, tariff = resolve_fare_table_tariff(
                fare_table_element, tariffs, filename
            )
            if tariff is None:
                continue
            ...
            sales_offer_package_refs = [
                sales_offer_package_ref.attrib["ref"]
                for sales_offer_package_ref in fare_table_element.findall(
                    "pricesFor/SalesOfferPackageRef"
                )
            ]
            sales_offer_package_options = [
                sales_offer_packages[ref]
                for ref in sales_offer_package_refs
                if ref in sales_offer_packages
            ]
            ...
            if columns_element is not None and rows_element is not None:
                ...
                # Stagecoach
                distance_matrix_elements = tariff_elements[tariff_ref].find(
                ...
                if distance_matrix_elements is not None:
                    ...
                    for price in fare_table_element.findall(
                        ...
                        price_key = (
                            ...
                            sales_offer_package.id if sales_offer_package else None,
                        )
                        if price_key not in prices:
                            prices[price_key] = models.Price.objects.create(
                                amount=amount,
                                tariff=tariff,
                                user_profile=user_profile,
                                sales_offer_package=sales_offer_package,
                        ...
                        models.DistanceMatrixElement.objects.create(
                            code=distance_matrix_element_ref,
                            tariff=tariff,
                            start_zone=fare_zones[start_zone],
                            end_zone=fare_zones[end_zone],
                    ...
                    for price in fare_table_element.findall("prices/*"):
                        amount = price.findtext("Amount")
                        if not amount:
                            continue
                        package_options = sales_offer_package_options or [
                            sales_offer_package
                        ]
                            ...
                            for cell_element in cells_element:
                                ...
                                if time_interval_price is not None:
                                    ...
                                    price_key = (
                                        ...
                                        tariff.id,
                                        user_profile.id if user_profile else None,
                                        sales_offer_package.id
                                        if sales_offer_package
                                        else None,
                                        time_interval.id if time_interval else None,
                                    )
                                    if price_key not in prices:
                                        prices[price_key] = models.Price.objects.create(
                                            amount=amount,
