...
class Ticket(models.Model):
    ...
    adult_price = models.DecimalField(max_digits=6, decimal_places=2, null=True, blank=True)
    child_price = models.DecimalField(max_digits=6, decimal_places=2, null=True, blank=True)
    days_valid_for = models.PositiveSmallIntegerField(null=True, blank=True)
    services = models.ManyToManyField(
        "busstops.Service", through="TicketAcceptance", blank=True
    )

    class Meta:
        ordering = ("operator__name", "ticket_type", "name", "id")
    ...
    def get_accepted_services(self):
        if hasattr(self, "_prefetched_objects_cache") and "ticketacceptance_set" in getattr(
            self, "_prefetched_objects_cache", {}
        ):
            return sorted(
                [
                    acceptance.service
                    for acceptance in self.ticketacceptance_set.all()
                    if acceptance.accepted
                ],
                key=lambda service: (service.line_name or "", service.description or "", service.pk),
            )
        return list(
            self.services.filter(ticketacceptance__accepted=True).order_by(
                "line_name", "description", "id"
            )
        )


class TicketAcceptance(models.Model):
